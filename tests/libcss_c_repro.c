...
static css_error node_name(void *pw, void *node, css_qname *qname)
{
    (void)pw;
    qname->ns = NULL;
    qname->name = lwc_string_ref((lwc_string *) node);
    return CSS_OK;
}
...
static css_error node_has_name(void *pw, void *node, const css_qname *qname, bool *match)
{
    (void)pw;
    *match = false;
    return lwc_string_caseless_isequal((lwc_string *) node, qname->name, match) == lwc_error_ok ? CSS_OK : CSS_NOMEM;
}
...
int main(void)
{
    ...
    if (lwc_intern_string("window", 6, &node) != lwc_error_ok) {
